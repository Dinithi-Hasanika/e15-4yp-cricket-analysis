...
for t in sub:
    ...
    isempty = df.empty
    if(isempty):
        result_grams = result_grams.append({'player-1':new_list[0], 'player-2':new_list[1],'player-3':new_list[2], 'avg':0.0}, ignore_index=True)
        continue
    ...
    result_grams = result_grams.append({'player-1':new_list[0], 'player-2':new_list[1], 'player-3':new_list[2], 'avg':avg}, ignore_index=True)
...
result_grams.to_csv('F:/8TH SEM/CO425-FYP/Zimbabwe/Combined averages/zimbabwe_3g.csv',sep=',')        
